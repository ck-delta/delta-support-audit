...
export const COMPARE_PROMPT = `You are auditing a support article from delta.exchange/support (hosted on Freshdesk) against authoritative source-of-truth content from Delta Exchange's official guides (\`guides.delta.exchange\`) and developer docs (\`docs.delta.exchange\`).
...
- **API base URL / hostname differences.** Do NOT flag when the only mismatch is the host/subdomain of an API URL — for example \`api.delta.exchange\` vs \`api.india.delta.exchange\`, \`socket.delta.exchange\` vs \`socket.india.delta.exchange\`, or any equivalent host variant where the path (\`/v2/products\`, \`/v2/tickers\`, etc.) and behavior are the same. These are deployment-routing differences, not factual contradictions about how the API behaves. Skip the entire finding — do not even flag it as P2.
- **Time-zone equivalencies.** Do NOT flag time differences that are equivalent under IST = UTC + 5:30. If support says "8:00 AM UTC" and SoT says "1:30 PM IST", or support says "every 8 hours at 12pm/8pm/4am IST" and SoT says "every 8 hours at 06:30/14:30/22:30 UTC", these describe the same moment and are NOT contradictions. Convert both to a canonical form (UTC) before comparing. Only flag if the actual underlying time differs after conversion.
...
export const CONFLICT_PROMPT = `You are checking whether two pieces of Delta Exchange's official documentation contradict each other. One is from \`guides.delta.exchange\` (user-facing rule book), the other from \`docs.delta.exchange\` (developer/API reference).
...
export const COVERAGE_PROMPT = `You are checking whether a topic documented in Delta Exchange's source-of-truth content (guides or docs) is adequately covered in the public Freshdesk support center used by Delta Exchange India users.
